...
run-pipeline-job: # Run only the pipeline container. Requires rest of stack to be up already
	docker compose -f docker/compose.yaml run pipeline_job && docker compose -f docker/compose.yaml logs pipeline_job -f

run-web-stack:  # Run the stack without starting a data pipeline job
	docker compose -f docker/compose.yaml up web db -d

run-full-stack:  # Run the full Docker compose stack
	docker compose -f docker/compose.yaml up -d && docker compose -f docker/compose.yaml logs pipeline_job -f
